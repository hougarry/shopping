        ...
        <li v-for="item in list" :key="item.productId">
          <el-popover placement="top">
            ...
            <div style="text-align: right; margin: 10px 0 0">
              <el-button type="primary" size="mini" @click="deleteCollect(item.productId)">是</el-button>
            </div>
  ...
  export default {
    ...
    methods: {
      deleteCollect(product_id) {
        this.$axios.delete(`/api/collect/user/${product_id}/${this.$store.getters.getUser.userId}`)
          .then(res => {
            if (res.data.code === "001") {
              // Emit an event instead of mutating 'list'
              this.$emit('delete-item', product_id);
              // Notify success
