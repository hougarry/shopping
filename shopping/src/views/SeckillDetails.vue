 ...
 <template>
      ...
      <div class="main">
        ...
        <div class="content">
          ...
          <!-- <p class="intro">{{productDetails.productIntro}}</p> -->
            <p class="store">开始时间: <span>{{ dateFormat(productDetails.startTime) }}</span></p>
            <p class="store">结束时间: <span>{{ dateFormat(productDetails.endTime) }}</span></p>
          <div class="price">
  ...
  export default {
    ...
    methods: {
      ...mapActions(["notifySucceed", "notifyError", "setShowLogin"]),
      // 获取商品详细信息
      getDetails(val) {
        this.$axios
